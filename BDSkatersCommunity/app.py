...
##################################
# Database Connection
engine = create_engine('sqlite:///BDSkaters.db')
...
def dashboard():
    ...
    elif current_user.UserType == 'guest':
        return render_template('guest.html')

        
    elif current_user.UserType == 'buyer':
...
@app.route('/events')
def events():
    video_form = EventForm()
    return render_template('events.html', video_form=video_form)
...
def add_video():
    ...
    if request.method == 'POST':
        if video_form.validate_on_submit():
            newEvent = Events(EventName=video_form.EventName.data, EmailAddress=video_form.EmailAddress.data, EventPost = video_form.EventPost.data)
